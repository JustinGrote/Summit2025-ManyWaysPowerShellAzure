...
param(
	...
	[string]$Name = $ENV:SOURCENAME ?? 'PowerShell',
	[ValidateNotNullOrEmpty()]
...
Trace-AICommand -Name ("ManyWays-$Name" -replace ' ') {
	...
	try {
		...
		Write-Verbose 'Connecting to Azure with:'
		$azConnectParams | Format-List | Out-String | Write-Verbose
		...
		# Get existing tags
		$existingTags = (Get-AzTag -ResourceId $resourceGroup.ResourceId).Properties.TagsProperty

		# Merge existing tags with new tags (new tags will override existing ones with the same name)
		if ($existingTags) {
		...
		# Update the resource with merged tags
		...
		Write-Verbose 'Connecting to Microsoft Graph with:'
		$connectParams | Format-List | Out-String | Write-Verbose
		...
		Write-Verbose "Creating new user: $($newUser.DisplayName) ($($newUser.UserPrincipalName))"
